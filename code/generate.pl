      ...
      foreach my $i (0..(scalar split(';',$db->{relation}{$w}{'tags'}{destination})-1)) {
        ...
        if($format ne 'json') {
          ...
          if ($db->{relation}{$w}{'tags'}{'osmc:symbol'}) {
            ...
            $o .= "<div class=\"symbol\"><img src=\"/OsmDestinationSigns/osmc/generate.pl?osmc=".$osmc."&opt=rectborder&size=32&out=svg\"></div>";
            }
          elsif($s->{waysymbol} && scalar @{$s->{waysymbol}}) {
            $o .= "<div class=\"symbol\">";
            foreach my $osmc (@{$s->{waysymbol}}) {
              $o .= "<img src=\"/OsmDestinationSigns/osmc/generate.pl?osmc=".$osmc."&opt=rectborder&size=32&out=svg\">";            
              }
            $o .= "</div>";  
            }
          $o .= "</div>";
